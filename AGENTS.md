# AGENTS.md - AI Agent Guidelines for gdcli

This document provides guidance for AI agents working with the gdcli codebase.

## Project Overview

gdcli is a command-line interface for Google Workspace APIs (Drive, Docs, Sheets, Slides). It follows the same patterns and conventions as [gmcli](https://github.com/badlogic/gmcli).
...
└── services/
    ...
    ├── drive-service.ts   # Google Drive API operations
    ├── docs-service.ts    # Google Docs API operations
    ├── sheets-service.ts  # Google Sheets API operations
    └── slides-service.ts  # Google Slides API operations
```
...
### Authentication Flow
- Credentials stored in `~/.gdcli/credentials.json`
- Account tokens stored in `~/.gdcli/accounts.json`
...
- Commands follow: `gdcli <email> <service> <command> [options]`
...
vi.mock("googleapis", () => ({
  google: {
    serviceName: vi.fn(() => mockServiceInstance),
  },
}));
```
...
- `marked` - Markdown parsing (for future doc import)
...
- Slides API v1: `https://www.googleapis.com/auth/presentations`

### Key API Concepts
- **Drive**: Files, folders, permissions, MIME types
- **Docs**: Documents with structural elements (paragraphs, tables, lists)
- **Sheets**: Spreadsheets with sheets, ranges (A1 notation), values
- **Slides**: Presentations with pages (slides), page elements, layouts
